...
export const SignupForm: React.FC = () => {
    ...
    const isFormComplete = email && username && password && passwordConfirm && agreeToTerms;
    return (
        <SmallFormContainer>
           <Stack component={"form"} onSubmit={handleSubmit(onSubmit)}>
               ...
               <Controller name="email" rules={{
                   ...
                   <TextInput {...field} label="メールアドレス" placeholder="example@example.com" error={!!errors.email}/>
               )}/>
               {errors.email?.message && (
                ...
                <Controller name="username" rules={{
                    ...
                    pattern: {
                        ...
                        <TextInput {...field} label="ユーザー名" placeholder="username" error={!!errors.username}/>
                    )}/>
                ...
                <Controller name="password" rules={{
                    ...
                    validate: {
                        hasNumber: value => /\d/.test(value) || "少なくとも1つの数字を含む必要があります",
                        hasSpecialCharacter: value => /[!@#$%^&*(),.?":{}|<>]/.test(value) || "少なくとも1つの特殊文字を含む必要があります",
                        hasUpperCase: value => /[A-Z]/.test(value) || "少なくとも1つの大文字を含む必要があります",
                        hasLowerCase: value => /[a-z]/.test(value) || "少なくとも1つの小文字を含む必要があります"
                    }
                }} control={control} render={({field}) => (
                    <TextInput {...field} label="パスワード" type="password" error={!!errors.password}/>
                )}/>
                {errors.password?.message && (
                    <StyledFormErrorText>{errors.password?.message}</StyledFormErrorText>
                )}
                <Controller name="passwordConfirm" rules={{
                    ...
                    <TextInput {...field} type="password" label="パスワード(確認)" error={!!errors.passwordConfirm}/>
                )}/>
                {errors.passwordConfirm?.message && (
                    <StyledFormErrorText>{errors.passwordConfirm?.message}</StyledFormErrorText>
                )}
               <Controller
                   name="agreeToTerms"
                   control={control}
               ...
               />
                {errors.agreeToTerms?.message && (
                     <StyledFormErrorText>{errors.agreeToTerms?.message}</StyledFormErrorText>
                )}
                <StyledPrimaryButton
